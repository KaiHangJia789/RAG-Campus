...
from extract_table import extract_table_descriptions
# 日志配置
logging.basicConfig(
    level=logging.INFO,
...
def process_pdf_to_chroma(pdf_path: str, chroma_dir: str) -> Chroma:
    ...
    # 2. 表格描述块（新增）
    table_descriptions = extract_table_descriptions(pdf_path)
    logger.info(f"从 PDF 中提取到 {len(table_descriptions)} 条表格描述")
    for desc in table_descriptions:
        # 描述的格式是 "(第3页) xxx" ，简单提取页码
        page = "未知"
        if desc.startswith("(第") and "页)" in desc:
            try:
                page_str = desc.split("(第")[1].split("页)")[0]
                page = int(page_str)
            except:
                pass
        chunks.append(
    ...
    logger.info(f"总共生成 {len(chunks)} 个块（含表格描述）")
